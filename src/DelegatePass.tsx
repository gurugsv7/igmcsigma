...
const DELEGATE_TIERS = [
  {
    name: "Student Delegate",
    price: "₹1500",
    benefits: [
      "Entry to all scientific sessions",
      "Entry to all workshops (with separate registration)",
      "Conference kit",
      "Lunch & refreshments (all days)",
...
const DelegatePass: React.FC = () => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Append user info to hidden fields for EmailJS
    const formElement = formRef.current;
    (formElement.elements.namedItem("delegate_tier") as HTMLInputElement).value = form.tier;
    ...
    emailjs.sendForm(
      ...
      (error) => {
        setIsSubmitting(false);
        setError("Failed to send delegate pass email. Please try again.");
      }
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-8 pb-24 px-6">
        <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-400/30 rounded-2xl p-8 max-w-md text-center">
          <h2 className="text-2xl font-bold text-green-300 mb-4">Delegate Pass Registration Successful!</h2>
          <p className="text-gray-300 mb-4">
            Thank you for registering for the <span className="text-cyan-300 font-semibold">{form.tier}</span>.<br />
            You will receive a confirmation email soon.
          </p>
          <div className="bg-black/20 rounded-lg p-3 mb-4">
            <p className="text-sm text-gray-400">Registration ID</p>
            <p className="text-cyan-300 font-mono">DELEGATE-{Date.now().toString().slice(-6)}</p>
          </div>
          <a
            href="/"
  ...
  return (
    <div className="min-h-screen pt-8 pb-24 px-6">
      <div className="max-w-2xl mx-auto bg-gradient-to-br from-cyan-900/40 to-purple-900/40 border border-cyan-400/20 rounded-2xl p-6 mb-8">
        <h2 className="text-3xl font-bold text-center text-cyan-300 mb-2">Delegate Pass Registration</h2>
        <p className="text-lg text-cyan-200 text-center mb-6">
          Choose your delegate tier and enjoy exclusive conference benefits!
        </p>
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          {DELEGATE_TIERS.map((tier) => (
            <div
              key={tier.name}
              className={`rounded-xl border border-cyan-400/30 bg-cyan-800/20 p-4 shadow-md ${
                form.tier === tier.name ? "ring-2 ring-cyan-400" : ""
              }`}
              onClick={() => setForm((prev) => ({ ...prev, tier: tier.name }))}
              style={{ cursor: "pointer" }}
            >
              <h3 className="text-xl font-bold text-cyan-300 mb-2">{tier.name}</h3>
              <div className="text-cyan-200 font-semibold mb-2">{tier.price}</div>
              <ul className="text-cyan-100 text-sm list-disc pl-5 mb-2">
                {tier.benefits.map((b, i) => (
                  <li key={i}>{b}</li>
                ))}
              </ul>
              {form.tier === tier.name && (
                <div className="text-green-300 font-semibold text-center mt-2">Selected</div>
              )}
            </div>
          ))}
        </div>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4" encType="multipart/form-data">
          ...
          <input type="hidden" name="delegate_tier" />
          ...
          <div className="my-4">
            <div className="bg-cyan-900/30 border border-cyan-400/20 rounded-lg p-3 mb-2 text-cyan-200 text-sm">
              ...
              <ol className="list-decimal pl-5 mt-1 mb-2">
                ...
                <li>Enter the correct amount as per your delegate tier.</li>
